...
parser = argparse.ArgumentParser(description='PyTorch CIFAR100 PGD Attack Evaluation')
...
def _pgd_whitebox(model,
    ...
    # MODIFIED: Variable已废弃
    X_pgd = X.clone().detach().requires_grad_(True)
    if args.random:
        random_noise = torch.FloatTensor(*X_pgd.shape).uniform_(-epsilon, epsilon).to(device)
        # MODIFIED: Variable已废弃
        X_pgd = (X_pgd.data + random_noise).clone().detach().requires_grad_(True)
    ...
    for _ in range(num_steps):
        ...
        # MODIFIED: Variable已废弃
        X_pgd = torch.clamp(X_pgd, 0, 1.0).clone().detach().requires_grad_(True)
...
def eval_adv_test_whitebox(model, device, test_loader):
    ...
    for data, target in test_loader:
        ...
        # pgd attack
        # MODIFIED: Variable已废弃
        X, y = data.clone().detach().requires_grad_(True), target.clone().detach()
...
def main():
    ...
    if args.white_box_attack:
        ...
        model_target = WideResNet(num_classes=100).to(device)
        model_target.load_state_dict(torch.load(args.target_model_path))
        model_source = WideResNet(num_classes=100).to(device)
