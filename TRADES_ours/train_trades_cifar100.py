...
parser = argparse.ArgumentParser(description='PyTorch CIFAR100 TRADES Adversarial Training')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.save_freq == 0 or epoch == args.epochs:
            ...
            torch.save(model.state_dict(),
                       os.path.join(model_dir, 'model-cifar100-wideres-epoch{}.pt'.format(epoch)))
            torch.save(optimizer.state_dict(),
                       os.path.join(model_dir, 'opt-cifar100-wideres-checkpoint_epoch{}.tar'.format(epoch)))
